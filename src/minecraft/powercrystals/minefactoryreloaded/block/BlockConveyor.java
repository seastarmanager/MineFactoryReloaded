...
public class BlockConveyor extends BlockContainer implements IConnectableRedNet {
    ...
    public void registerIcons(IconRegister ir) {
        for (int i = 0; i < _names.length; i++) {
            _iconsActive[i] = ir.registerIcon(MineFactoryReloadedCore.modId + ":" + getUnlocalizedName() + ".active." + _names[i]);
            _iconsStopped[i] = ir.registerIcon(MineFactoryReloadedCore.modId + ":" + getUnlocalizedName() + ".stopped." + _names[i]);
        }
    }
