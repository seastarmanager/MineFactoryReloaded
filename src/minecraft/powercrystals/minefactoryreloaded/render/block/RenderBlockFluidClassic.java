...
public class RenderBlockFluidClassic implements ISimpleBlockRenderingHandler
{
	...
	public float getFluidHeightForRender(IBlockAccess world, int x, int y, int z, BlockFluidClassic block)
	{
		if(world.getBlockId(x, y, z) == block.blockID)
		{
			...
			if(world.getBlockId(x, y - block.getDensityDir(), z) == block.blockID)
			{
				return 1;
			}
			if(world.getBlockMetadata(x, y, z) == block.getQuantaPerBlock() - 1)
			{
				return 0.875F;
			}
		}
		return !world.getBlockMaterial(x, y, z).isSolid() && world.getBlockId(x, y - block.getDensityDir(), z) == block.blockID ? 1 : block.getQuantaPercentage(
				world, x, y, z) * 0.875F;
	}
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
	{
		...
		boolean renderTop = world.getBlockId(x, y - theFluid.getDensityDir(), z) != theFluid.blockID;

		boolean renderBottom = block.shouldSideBeRendered(world, x, y + theFluid.getDensityDir(), z, 0)
				&& world.getBlockId(x, y + theFluid.getDensityDir(), z) != theFluid.blockID;
